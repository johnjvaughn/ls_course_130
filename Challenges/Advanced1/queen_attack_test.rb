...
class QueensTest < Minitest::Test
  ...
  def test_cannot_be_outside_valid_range
    #skip
    assert_raises ArgumentError do
      Queens.new(white: [2, 4], black: [2, 8])
    end
  end
...
_ _ _ _ _ _ _ _
  ...
  def test_cant_attack
    #skip
    queens = Queens.new(white: [5, 1], black: [1, 6])
    refute queens.attack?
  end

  def test_cant_attack_again
    #skip
    queens = Queens.new(white: [5, 1], black: [0, 3])
    refute queens.attack?
  end
end
